...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--dynamic-slots"},
        {"--no-dynamic-slots"},
        "allow the server to create more concurrent slots than --parallel when the KV block pool has space, up to the n_seq_max limit (default: disabled; requires --kv-unified)",
        [](common_params & params, bool value) {
            params.dynamic_slots = value;
        }
    ).set_env("LLAMA_ARG_DYNAMIC_SLOTS").set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--context-shift"},
        {"--no-context-shift"},
