...
- [x] Defer per paged capacity: **FIXATO** (commit `1a050c218`). `pop_deferred_task()` era chiamato SOLO da `server_slot::callback_on_release` (`server-context.cpp:907`), mai in paged mode (no slots, `initial_slot_count()`→0). Aggiunta callback `PagedRequestLauncherConfig::pop_deferred_task` chiamata nel `callback_on_release` paged. Verificato a debug verbosity (cap=4, 6 concorrenti): 2 deferred, 2 ri-poppate al release.
- [x] **Stall a piena capacità: FIXATO** (commit `854485cf8`). Quando il set ammesso == `max-num-seqs`, lo scheduler entrava in loop `[paged] no tokens to decode`. Causa: `can_admit` nel schedule builder (`paged_schedule_builder.cpp`) ri-applicava il cap di admission a richieste **già lanciate** che tengono il lease seq-id; il cap conta le richieste attive, quindi valutare una richiesta già attiva la double-counta (`running+1 > cap`) e la deferiva all'infinito → mai prefill → mai decode → deadlock. Fix: bypass admission per `req.is_processing()` (già ammesse al launch, serve solo scheduling). Verificato: N=4 era 0/4 → 4/4; cap=4 con 6 e 12 concorrenti → 6/6 e 12/12; actual-len 6/6; legacy 6/6.
