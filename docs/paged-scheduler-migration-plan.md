...
- [ ] **NUOVO BUG TROVATO (separato, pre-esistente): stall a piena capacità.** Quando il set ammesso == `max-num-seqs` (es. cap=4 con 4+ richieste truly-simultaneous), lo scheduler paged entra in loop di `[paged] no tokens to decode` e le richieste vanno in timeout. N=1/N=3 (sotto cap) ok, N=4 (=cap) stalla. Non è il defer (succede anche senza defer, a esatta capacità). Probabile bug in schedule builder / prefill-decode policy che produce batch a zero token a piena capacità. **Da investigare separatamente** — non toccato dal fix defer né dai refactor Fase 1-4.
